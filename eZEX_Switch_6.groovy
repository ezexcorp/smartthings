 ...
 */
metadata {
    definition(name: "eZEX Switch 6", namespace: "eZEX", author: "eZEX Corp", ocfDeviceType: "oic.d.switch") {
        capability "Actuator"
        ...
        fingerprint profileId: "0104", inClusters: "0000, 0003, 0004, 0005, 0006", manufacturer: "eZEX Corp", model: "E220-KR6N0Z0-HA", deviceJoinName: "eZEX Switch 6"
    }
    ...
    tiles(scale: 2) {
        ...
        main "switch"
        details(["switch", "switch4", "switch1", "on", "off", "switch5", "switch2", "switch6", "switch3", "refresh"])
    }
}
...
def checkStatus() {
    if (state.sw1 == "off" && state.sw2 == "off" && state.sw3 == "off" && state.sw4 == "off" && state.sw5 == "off" && state.sw6 == "off") {
        state.swAll = "off"
        state.swOn = "default"
        state.swOff = "disabled"
    } else {
        state.swAll = "on"
        if (state.sw1 == "on" && state.sw2 == "on" && state.sw3 == "on" && state.sw4 == "on" && state.sw5 == "on" && state.sw6 == "on") {
            state.swOn = "disabled"
        } else {
            state.swOn = "default"
        }
